...
def post_init_hook(env):
    ...
    for sequence, code in enumerate(["MASTER", "CHENG", "OILER", "DECK"], start=1):
        ranks[code] = _record(env, "sedar.crew.rank", f"rank_{code.lower()}", {
    ...
    for xmlid, name, owner, vessel_type, imo, tonnage in vessel_specs:
        vessels[xmlid] = _record(env, "sedar.client.vessel", xmlid, {
    ...
    tugs = {}
    for index, (code, name, tug_class, pull, availability) in enumerate(tug_specs, start=1):
        tugs[code] = _record(env, "sedar.tugboat", f"tug_{code}", {
    ...
    for rank_code, names in crew_names.items():
        for rank_index, name in enumerate(names):
            ...
            if rank_code == "MASTER": certificate_codes.append("COC-M")
            if rank_code == "CHENG": certificate_codes.append("COC-E")
            for certificate_code in certificate_codes:
                if employee_number == 21 and certificate_code == "MED":
                    continue
                expiry = "2027-08-31"
                if employee_number == 10 and certificate_code == "MED": expiry = "2026-07-31"
                if employee_number == 20 and certificate_code == "STCW": expiry = "2026-08-20"
